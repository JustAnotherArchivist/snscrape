...
class TelegramChannelScraper(snscrape.base.Scraper):
    ...
    def _soup_to_items(self, soup, pageUrl, onlyUsername=False):
        ...
        for post in reversed(posts):
            ...
            if message := post.find('div', class_='tgme_widget_message_text'):
                ...
                outlinks = []
            linkPreview = None
            if linkPreviewA := post.find('a', class_='time_widget_message_link_preview'):
                kwargs = {}
    ...
    def _get_entity(self):
        ...
        try:
            ...
            # If there are no posts, fall back to the channel info div, although that should never happen due to the
            # 'Channel created' entry.
            _logger.warning(
                'Could not find a post; extracting username from channel info div, which may not be capitalised '
                'correctly')
