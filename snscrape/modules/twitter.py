...
class Tweet(snscrape.base.Item):
    ...
    outlinksss = snscrape.base._DeprecatedProperty('outlinksss',
                                                   lambda self: ' '.join(self.outlinks) if self.outlinks else '',
                                                   'outlinks')
    tcooutlinksss = snscrape.base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(
        self.tcooutlinks) if self.tcooutlinks else '', 'tcooutlinks')
...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    def _ensure_guest_token(self, url=None):
        ...
        self._session.cookies.set('gt', self._guestTokenManager.token, domain='.twitter.com', path='/', secure=True,
                                  expires=self._guestTokenManager.setTime + 10800)
        ...
        while True:
            ...
            for instruction in obj['timeline']['instructions']:
                ...
                for entry in entries:
                    if entry['entryId'] == f'sq-cursor-{dir}' or entry['entryId'].startswith(f'cursor-{dir}-'):
                    ...
                    elif entry['entryId'].startswith(
                            'cursor-showMoreThreadsPrompt-'):  # E.g. 'offensive' replies button
                        promptCursor = entry['content']['operation']['cursor']['value']
                            ...
                            entry['entryId'] == f'sq-cursor-bottom' or entry['entryId'].startswith('cursor-bottom-')):
                        newBottomCursorAndStop = (entry['content']['operation']['cursor']['value'],
                                                  entry['content']['operation']['cursor'].get('stopOnEmptyResponse',
                                                                                              False))
    ...
    def _instructions_to_tweets(self, obj, includeConversationThreads=False):
        ...
        for instruction in obj['timeline']['instructions']:
            ...
            for entry in entries:
                ...
                elif includeConversationThreads and entry['entryId'].startswith('conversationThread-') and not entry[
                    'entryId'].endswith('-show_more_cursor'):
    ...
    def _instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
        if 'tweet' in entry['item']['content']:
            ...
            if entry['item']['content']['tombstone']['tweet']['id'] not in obj['globalObjects']['tweets']:
                _logger.warning(
                    f'Skipping tweet {entry["item"]["content"]["tombstone"]["tweet"]["id"]} which is not in globalObjects')
                return
            ...
            for medium in tweet['extended_entities']['media']:
                if medium['type'] == 'photo':
                    if '.' not in medium['media_url_https']:
                        _logger.warning(
                            f'Skipping malformed medium URL on tweet {kwargs["id"]}: {medium["media_url_https"]!r} contains no dot')
                        continue
                    ...
                    if medium['type'] == 'video':
                        ...
                        if (ext := medium['ext']) and (mediaStats := ext['mediaStats']) and isinstance(
    ...
    def _user_to_user(self, user):
        ...
        kwargs['description'] = self._render_text_with_urls(user['description'],
                                                            user['entities']['description'].get('urls'))
        kwargs['rawDescription'] = user['description']
        if user['entities']['description'].get('urls'):
            kwargs['descriptionUrls'] = [{'text': x.get('display_url'), 'url': x['expanded_url'], 'tcourl': x['url'],
                                          'indices': tuple(x['indices'])} for x in
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def __init__(self, query, cursor=None, top=False, lang='en', **kwargs):
        if not query.strip():
            raise ValueError('empty query')
        super().__init__(baseUrl='https://twitter.com/search?' + urllib.parse.urlencode(
            {'f': 'live', 'lang': lang, 'q': query, 'src': 'spelling_expansion_revert_click'}), **kwargs)
    ...
    def cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument('--top', action='store_true', default=False,
                               help='Enable fetching top tweets instead of live/chronological')
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _get_entity(self):
        ...
        params = {
            'variables': json.dumps({fieldName: self._username, 'withHighlightedLabel': True}, separators=(',', ':'))}
        ...
        return User(
            ...
            descriptionUrls=[{'text': x.get('display_url'), 'url': x['expanded_url'], 'tcourl': x['url'],
                              'indices': tuple(x['indices'])} for x in
            ...
            linkUrl=user['legacy']['entities']['url']['urls'][0]['expanded_url'] if 'url' in user['legacy'][
                'entities'] else None,
...
class TwitterProfileScraper(TwitterUserScraper):
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data(f'https://api.twitter.com/2/timeline/profile/{userId}.json', params,
                                       paginationParams):
            yield from self._instructions_to_tweets(obj)
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            for obj in self._iter_api_data(f'https://twitter.com/i/api/2/timeline/conversation/{self._tweetId}.json',
                                           params, paginationParams, direction=_ScrollDirection.BOTH):
            ...
            while queue:
                ...
                for obj in self._iter_api_data(f'https://twitter.com/i/api/2/timeline/conversation/{tweetId}.json',
                                               params, paginationParams, direction=_ScrollDirection.BOTH):
