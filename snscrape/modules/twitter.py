...
class Tweet(snscrape.base.Item):
    ...
    outlinks = snscrape.base._DeprecatedProperty('outlinks',
                                                 lambda self: [x.url for x in self.links] if self.links else [],
                                                 'links (url attribute)')
    outlinksss = snscrape.base._DeprecatedProperty('outlinksss', lambda self: ' '.join(
        x.url for x in self.links) if self.links else '', 'links (url attribute)')
    tcooutlinks = snscrape.base._DeprecatedProperty('tcooutlinks',
                                                    lambda self: [x.tcourl for x in self.links] if self.links else [],
                                                    'links (tcourl attribute)')
    tcooutlinksss = snscrape.base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(
        x.tcourl for x in self.links) if self.links else '', 'links (tcourl attribute)')
...
class UnifiedCard(Card):
    ...
    def __post_init__(self):
        if (self.components is not None) + (self.swipeableLayoutSlides is not None) + (
                self.collectionLayoutSlides is not None) != 1:
...
class User(snscrape.base.Item):
    ...
    renderedDescription: typing.Optional[
        str] = None  # Description as it's displayed on the web interface with URLs replaced
    ...
    descriptionUrls = snscrape.base._DeprecatedProperty('descriptionUrls', lambda self: self.descriptionLinks,
                                                        'descriptionLinks')
    linkUrl = snscrape.base._DeprecatedProperty('linkUrl', lambda self: self.link.url if self.link else None,
                                                'link.url')
    linkTcourl = snscrape.base._DeprecatedProperty('linkTcourl', lambda self: self.link.tcourl if self.link else None,
                                                   'link.tcourl')
    description = snscrape.base._DeprecatedProperty('description', lambda self: self.renderedDescription,
                                                    'renderedDescription')
...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    def _ensure_guest_token(self, url=None):
        if self._guestTokenManager.token is None:
            ...
            if not self._guestTokenManager.token:
                ...
                r = self._post('https://api.twitter.com/1.1/guest/activate.json', data=b'', headers=self._apiHeaders,
                               responseOkCallback=self._check_guest_token_response)
                o = r.json()
        ...
        self._session.cookies.set('gt', self._guestTokenManager.token, domain='.twitter.com', path='/', secure=True,
            ...
            for instruction in instructions:
                ...
                for entry in entries:
                    ...
                    if apiType is _TwitterAPIType.V2:
                        ...
                        while cursorContent.get('itemType') == 'TimelineTimelineItem' or cursorContent.get(
                                'entryType') == 'TimelineTimelineItem':
                            cursorContent = cursorContent['itemContent']
                        entryCursor, entryCursorStop = cursorContent['value'], cursorContent.get('stopOnEmptyResponse',
                                                                                                 None)
                    ...
                    elif entry['entryId'].startswith('cursor-showmorethreadsprompt-') or entry['entryId'].startswith(
                            'cursor-showmorethreads-'):
                        # E.g. 'offensive' replies and 'Show more replies' button
                        promptCursor = entryCursor
                    elif direction is _ScrollDirection.BOTH and bottomCursorAndStop is None and (
                            entry['entryId'] == 'sq-cursor-bottom' or entry['entryId'].startswith('cursor-bottom-')):
    ...
    def _count_tweets_and_users(self, entries):
        return sum(entry['entryId'].startswith('sq-I-t-') or entry['entryId'].startswith('tweet-') or entry[
            'entryId'].startswith('user-') for entry in entries)
        ...
        for instruction in obj['timeline']['instructions']:
            ...
            for entry in entries:
                if entry['entryId'].startswith('sq-I-t-') or entry['entryId'].startswith('tweet-'):
                    ...
                    yield self._user_to_user(
                        obj['globalObjects']['users'][entry['content']['item']['content']['user']['id']])
    ...
    def _make_tweet(self, tweet, user, retweetedTweet=None, quotedTweet=None, card=None, **kwargs):
            ...
            if inReplyToUserId == kwargs['user'].id:
                ...
                for u in tweet['entities']['user_mentions']:
                    if u['id_str'] == tweet['in_reply_to_user_id_str']:
                        kwargs['inReplyToUser'] = User(username=u['screen_name'],
        ...
        if tweet.get('place'):
            kwargs['place'] = Place(tweet['place']['id'], tweet['place']['full_name'], tweet['place']['name'],
    ...
    def _make_medium(self, medium, tweetId):
        if medium['type'] == 'photo':
            ...
            if '.' not in medium['media_url_https']:
                _logger.warning(
                    f'Skipping malformed medium URL on tweet {tweetId}: {medium["media_url_https"]!r} contains no dot')
                return
            ...
            for variant in medium['video_info']['variants']:
                variants.append(VideoVariant(contentType=variant['content_type'], url=variant['url'],
            ...
            if medium['type'] == 'video':
                ...
                if (ext := medium.get('ext')) and (mediaStats := ext.get('mediaStats')) and isinstance(
                        r := mediaStats['r'], dict) and 'ok' in r and isinstance(r['ok'], dict):
    ...
    def _make_card(self, card, apiType, tweetId):
        ...
        for key, value in messyBindingValues:
            ...
            if value['type'] == 'STRING':
                ...
                if key.endswith('_datetime_utc'):
                    bindingValues[key] = datetime.datetime.strptime(bindingValues[key], '%Y-%m-%dT%H:%M:%SZ').replace(
        ...
        if cardName in ('summary', 'summary_large_image', 'app', 'direct_store_link_app'):
        ...
        elif any(cardName.startswith(x) for x in ('poll2choice_', 'poll3choice_', 'poll4choice_')) and \
                cardName.split('_', 1)[1] in ('text_only', 'image', 'video'):
            kwargs = snscrape.utils.dict_map(bindingValues, {'end_datetime_utc': 'endDate',
                                                             'last_updated_datetime_utc': 'lastUpdateDate',
                                                             'duration_minutes': 'duration',
                                                             'counts_are_final': 'finalResults'})
            ...
            if cardName.endswith('_image'):
                kwargs['medium'] = Photo(previewUrl=bindingValues['image_small'],
                                         fullUrl=bindingValues['image_original'])
            elif cardName.endswith('_video'):
                variants = []
                variants.append(VideoVariant(contentType='application/x-mpegurl', url=bindingValues['player_hls_url'],
                                             bitrate=None))
                ...
                variants.append(
                    VideoVariant(contentType='text/xml', url=bindingValues['player_stream_url'], bitrate=None))
                kwargs['medium'] = Video(thumbnailUrl=bindingValues['player_image_original'], variants=variants,
                                         duration=int(bindingValues['content_duration_seconds']))

            return PollCard(**kwargs)
        elif cardName == 'player':
            return PlayerCard(**snscrape.utils.dict_map(bindingValues, {'title': 'title', 'description': 'description',
                                                                        'card_url': 'url',
                                                                        'player_image_original': 'imageUrl',
                                                                        'site': 'siteUser'}))
        elif cardName in ('promo_image_convo', 'promo_video_convo'):
            kwargs = snscrape.utils.dict_map(bindingValues,
                                             {'thank_you_text': 'thankYouText', 'thank_you_url': 'thankYouUrl',
                                              'thank_you_shortened_url': 'thankYouTcoUrl'})
            kwargs['actions'] = []
            for l in ('one', 'two', 'three', 'four'):
                if f'cta_{l}' in bindingValues:
                    kwargs['actions'].append(
                        PromoConvoAction(label=bindingValues[f'cta_{l}'], tweet=bindingValues[f'cta_{l}_tweet']))
            if 'image' in cardName:
                kwargs['medium'] = Photo(previewUrl=bindingValues['promo_image_small'],
                                         fullUrl=bindingValues['promo_image_original'])
                if 'cover_promo_image' in bindingValues:
                    kwargs['cover'] = Photo(previewUrl=bindingValues['cover_promo_image_small'],
                                            fullUrl=bindingValues['cover_promo_image_original'])
            elif 'video' in cardName:
                variants = []
                variants.append(VideoVariant(contentType=bindingValues['player_stream_content_type'],
                                             url=bindingValues['player_stream_url'], bitrate=None))
            ...
            if cardName == '745291183405076480:broadcast':
                keyMap = {**keyMap, 'broadcast_id': 'id', 'broadcast_url': 'url', 'broadcast_title': 'title',
                          'broadcast_thumbnail_original': 'thumbnailUrl'}
            else:
                keyMap = {**keyMap, 'id': 'id', 'url': 'url', 'title': 'title', 'description': 'description',
                          'total_participants': 'totalParticipants', 'full_size_thumbnail_url': 'thumbnailUrl'}
            ...
            kwargs = snscrape.utils.dict_map(bindingValues,
                                             {'event_id': 'id', 'event_title': 'title', 'event_category': 'category',
                                              'event_subtitle': 'description'})
            kwargs['id'] = int(kwargs['id'])
            kwargs['photo'] = Photo(previewUrl=bindingValues['event_thumbnail_small'],
                                    fullUrl=bindingValues.get('event_thumbnail_original') or bindingValues[
                                        'event_thumbnail'])
            return EventCard(event=Event(**kwargs))
        elif cardName == '3337203208:newsletter_publication':
            kwargs = snscrape.utils.dict_map(bindingValues,
                                             {'newsletter_title': 'title', 'newsletter_description': 'description',
                                              'newsletter_image_original': 'imageUrl', 'card_url': 'url',
                                              'revue_account_id': 'revueAccountId', 'issue_count': 'issueCount'})
            ...
            return AmplifyCard(
                ...
                video=Video(
                    ...
                    variants=[VideoVariant(url=bindingValues['amplify_url_vmap'],
                                           contentType=bindingValues.get('player_stream_content_type'), bitrate=None)],
                ),
            )
        elif cardName == 'appplayer':
            kwargs = snscrape.utils.dict_map(bindingValues, {'title': 'title', 'app_category': 'appCategory',
                                                             'player_owner_id': 'playerOwnerId', 'site': 'siteUser'})
            kwargs['playerOwnerId'] = int(kwargs['playerOwnerId'])
            variants = []
            variants.append(
                VideoVariant(contentType='application/x-mpegurl', url=bindingValues['player_hls_url'], bitrate=None))
            if 'vmap' not in bindingValues['player_url']:
                _logger.warning(f'Non-VMAP URL in {cardName} player_url on tweet {tweetId}')
            variants.append(VideoVariant(contentType='text/xml', url=bindingValues['player_url'], bitrate=None))
            kwargs['video'] = Video(thumbnailUrl=bindingValues['player_image_original'], variants=variants,
                                    duration=int(bindingValues['content_duration_seconds']))
            ...
            if bindingValues['cta'] not in ctas:
                ...
                return
            return MessageMeCard(
                **snscrape.utils.dict_map(bindingValues, {'recipient': 'recipient', 'card_url': 'url'}),
                buttonText=ctas[bindingValues['cta']])
            ...
            for k, v in o['component_objects'].items():
                if v['type'] == 'details':
                    co = UnifiedCardDetailComponentObject(content=v['data']['title']['content'],
                                                          destinationKey=v['data']['destination'])
                elif v['type'] == 'media':
                    co = UnifiedCardMediumComponentObject(mediumKey=v['data']['id'],
                                                          destinationKey=v['data']['destination'])
                    ...
                    if not all(b['type'] == 'cta' for b in v['data']['buttons']):
                        ...
                        return
                    buttons = [UnifiedCardButton(
                        text=b['action'][0].upper() + re.sub('[A-Z]', lambda x: f' {x[0]}', b['action'][1:]),
                        destinationKey=b['destination']) for b in v['data']['buttons']]
                    co = UnifiedCardButtonGroupComponentObject(buttons=buttons)
                elif v['type'] == 'swipeable_media':
                    media = [UnifiedCardSwipeableMediaMedium(mediumKey=m['id'], destinationKey=m['destination']) for m
                             in v['data']['media_list']]
                    co = UnifiedCardSwipeableMediaComponentObject(media=media)
                elif v['type'] == 'app_store_details':
                    co = UnifiedCardAppStoreComponentObject(appKey=v['data']['app_id'],
                                                            destinationKey=v['data']['destination'])
                    ...
                    co = UnifiedCardTwitterCommunityDetailsComponentObject(
                        ...
                        destinationKey=v['data']['destination'],
                        membersFacepile=[self._user_to_user(u) for u in
                                         map(o['users'].get, v['data']['members_facepile']) if u],
                    )
                else:
            ...
            if 'app_store_data' in o:
                ...
                for k, v in o['app_store_data'].items():
                    ...
                    for var in v:
                        ...
                        vKwargs['url'] = f'https://play.google.com/store/apps/details?id={var["id"]}' if var[
                                                                                                             'type'] == 'android_app' else f'https://itunes.apple.com/app/id{var["id"]}'
                        if 'iconMediumKey' in vKwargs and vKwargs['iconMediumKey'] not in kwargs['media']:
                            ...
                            _logger.warning(
                                f'Tweet {tweetId} contains an app icon medium key {vKwargs["iconMediumKey"]!r} on app {vKwargs["type"]!r}/{vKwargs["id"]!r}, but the corresponding medium is missing; dropping')
            ...
            if 'layout' in o:
                if o['layout']['type'] == 'swipeable':
                    kwargs['swipeableLayoutSlides'] = [
                        UnifiedCardSwipeableLayoutSlide(mediumComponentKey=v[0], componentKey=v[1]) for v in
                        o['layout']['data']['slides']]
                elif o['layout']['type'] == 'collection':
                    kwargs['collectionLayoutSlides'] = [
                        UnifiedCardCollectionLayoutSlide(detailsComponentKey=v[0], mediumComponentKey=v[1]) for v in
                        o['layout']['data']['slides']]
                else:
            ...
            if card.swipeableLayoutSlides and not all(
                    s.mediumComponentKey in card.componentObjects and s.componentKey in card.componentObjects for s in
                    card.swipeableLayoutSlides):
                missingParts.add('components')
            if any(c.destinationKey not in card.destinations for c in card.componentObjects.values() if
                   hasattr(c, 'destinationKey')):
                missingParts.add('destinations')
            if any(b.destinationKey not in card.destinations for c in card.componentObjects.values() if
                   isinstance(c, UnifiedCardButtonGroupComponentObject) for b in c.buttons):
                missingParts.add('destinations')
            mediaKeys = []
            ...
            mediaKeys.extend(
                a.iconMediumKey for l in (card.apps.values() if card.apps is not None else []) for a in l if
                a.iconMediumKey is not None)
    ...
    def _tweet_to_tweet(self, tweet, obj):
        ...
        if 'retweeted_status_id_str' in tweet:
            kwargs['retweetedTweet'] = self._tweet_to_tweet(
                obj['globalObjects']['tweets'][tweet['retweeted_status_id_str']], obj)
        if 'quoted_status_id_str' in tweet and tweet['quoted_status_id_str'] in obj['globalObjects']['tweets']:
            kwargs['quotedTweet'] = self._tweet_to_tweet(obj['globalObjects']['tweets'][tweet['quoted_status_id_str']],
                                                         obj)
        if 'card' in tweet:
    ...
    def _make_tombstone(self, tweetId, info):
        ...
        if info and (text := info.get('richText', info['text'])):
            return Tombstone(
                id=tweetId,
                text=text['text'],
                textLinks=[TextLink(text=text['text'][x['fromIndex']:x['toIndex']], url=x['ref']['url'], tcourl=None,
                                    indices=(x['fromIndex'], x['toIndex'])) for x in text['entities']],
            )
        else:
    ...
    def _graphql_timeline_tweet_item_result_to_tweet(self, result, tweetId=None):
        ...
        if 'retweeted_status_result' in tweet:
            ...
            kwargs['retweetedTweet'] = self._graphql_timeline_tweet_item_result_to_tweet(
                tweet['retweeted_status_result']['result'])
        if 'quoted_status_result' in result:
            if 'result' not in result['quoted_status_result']:
                _logger.warning(
                    f'quoted_status_result for {tweet["quoted_status_id_str"]} without an actual result on tweet {self._get_tweet_id(tweet)}, using TweetRef')
                kwargs['quotedTweet'] = TweetRef(int(tweet['quoted_status_id_str']))
            else:
                kwargs['quotedTweet'] = self._graphql_timeline_tweet_item_result_to_tweet(
                    result['quoted_status_result']['result'], tweetId=int(tweet['quoted_status_id_str']))
        elif result.get('quotedRefResult'):
            if result['quotedRefResult']['result']['__typename'] == 'TweetTombstone':
                kwargs['quotedTweet'] = self._graphql_timeline_tweet_item_result_to_tweet(
                    result['quotedRefResult']['result'], tweetId=int(tweet['quoted_status_id_str']))
            else:
                qTweet = result['quotedRefResult']['result']
                if result['quotedRefResult']['result']['__typename'] not in ('Tweet', 'TweetWithVisibilityResults'):
                    _logger.warning(
                        f'Unknown quotedRefResult type {result["quotedRefResult"]["result"]["__typename"]!r} on tweet {self._get_tweet_id(tweet)}, using TweetRef')
            ...
            if tweet['quoted_status_id_str'] != tweet.get('retweeted_status_result', {}).get('result', {}).get(
                    'quoted_status_result', {}).get('result', {}).get('rest_id'):
        ...
        for instruction in instructions:
            ...
            for entry in instruction['entries']:
                if entry['entryId'].startswith('tweet-'):
                    ...
                    if entry['content']['entryType'] == 'TimelineTimelineItem' and entry['content']['itemContent'][
                        ...
                        yield self._graphql_timeline_tweet_item_result_to_tweet(
                            entry['content']['itemContent']['tweet_results']['result'], tweetId=tweetId)
                    else:
                        ...
                        for item in entry['content']['items']:
                            ...
                            if item['item']['itemContent']['itemType'] == 'TimelineTweet':
                                if 'result' in item['item']['itemContent']['tweet_results']:
                                    yield self._graphql_timeline_tweet_item_result_to_tweet(
                                        item['item']['itemContent']['tweet_results']['result'], tweetId=tweetId)
                                else:
                                    yield TweetRef(id=tweetId)
                elif includeConversationThreads and entry['entryId'].startswith(
                        'conversationthread-'):  # TODO show more cursor?
                    for item in entry['content']['items']:
    ...
    def _render_text_with_urls(self, text, urls):
        ...
        urlsSorted = sorted(urls,
                            key=lambda x: x['indices'][0])  # Ensure that they're in left to right appearance order
        assert all(
            url['indices'][1] <= nextUrl['indices'][0] for url, nextUrl in zip(urls, urls[1:])), 'broken URL indices'
    ...
    def _user_to_user(self, user, id_=None):
        ...
        kwargs['renderedDescription'] = self._render_text_with_urls(user['description'],
        ...
        if user.get('url'):
            ...
            kwargs['link'] = TextLink(text=entity.get('display_url'), url=entity.get('expanded_url', user['url']),
    ...
    def _graphql_user_results_to_user_ref(self, obj):
        ...
        kwargs = {}
        if 'result' in obj and obj['result']['__typename'] == 'UserUnavailable' and 'unavailable_message' in obj[
            'result']:
            kwargs['text'] = obj['result']['unavailable_message']['text']
            kwargs['textLinks'] = [
                TextLink(text=kwargs['text'][x['fromIndex']:x['toIndex']], url=x['ref']['url'], tcourl=None,
                         indices=(x['fromIndex'], x['toIndex'])) for x in
                obj['result']['unavailable_message']['entities']]
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def __init__(self, query, *, cursor=None, mode=TwitterSearchScraperMode.LIVE, top=None, maxEmptyPages=20, **kwargs):
        ...
        super().__init__(baseUrl='https://twitter.com/search?' + urllib.parse.urlencode(
            {'f': 'live', 'lang': 'en', 'q': query, 'src': 'spelling_expansion_revert_click'}), **kwargs)
        ...
        if top is not None:
            ...
            warnings.warn(f'`top` argument is deprecated, use `mode = {replacement}` instead of `top = {bool(top)}`',
                          snscrape.base.DeprecatedFeatureWarning, stacklevel=2)
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data('https://api.twitter.com/2/search/adaptive.json', _TwitterAPIType.V2, params,
                                       paginationParams, cursor=self._cursor):
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        group.add_argument('--top', action='store_true', default=False,
                           help='Search top tweets instead of live/chronological')
        group.add_argument('--user', action='store_true', default=False, help='Search users instead of tweets')
        subparser.add_argument('--max-empty-pages', dest='maxEmptyPages', metavar='N', type=int, default=20,
                               help='Stop after N empty pages from Twitter; set to 0 to disable')
        subparser.add_argument('query', type=snscrape.base.nonempty_string('query'), help='A Twitter search string')

    @classmethod
    def _cli_from_args(cls, args):
        return cls._cli_construct(args, args.query, cursor=args.cursor,
                                  mode=TwitterSearchScraperMode._cli_from_args(args), maxEmptyPages=args.maxEmptyPages)
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _get_entity(self):
        ...
        renderedDescription = self._render_text_with_urls(rawDescription,
        ...
        if user['legacy'].get('url'):
            ...
            link = TextLink(text=entity.get('display_url'), url=entity.get('expanded_url', user['legacy']['url']),
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument('--user-id', dest='isUserId', action='store_true', default=False,
                               help='Use user ID instead of username')
            ...
            if not gotPinned:
                for instruction in instructions:
                    if instruction['type'] == 'TimelinePinEntry':
                        gotPinned = True
                        tweetId = int(instruction['entry']['entryId'][6:]) if instruction['entry'][
                            'entryId'].startswith('tweet-') else None
                        yield self._graphql_timeline_tweet_item_result_to_tweet(
...
class TwitterHashtagScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('hashtag', type=snscrape.base.nonempty_string('hashtag'),
                               help='A Twitter hashtag (without #)')
...
class TwitterCashtagScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('cashtag', type=snscrape.base.nonempty_string('cashtag'),
                               help='A Twitter cashtag (without $)')
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, params, paginationParams,
                                           direction=_ScrollDirection.BOTH):
                if not obj['data']:
                    continue
                yield from self._graphql_timeline_instructions_to_tweets(
                    obj['data']['threaded_conversation_with_injections_v2']['instructions'],
            ...
            while queue:
                ...
                for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, thisParams, thisPagParams,
                                               direction=_ScrollDirection.BOTH):
                    if not obj['data']:
                        continue
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        group.add_argument('--recurse', '--recursive', action='store_true', default=False,
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    @classmethod
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('list', type=snscrape.base.nonempty_string('list'),
                               help='A Twitter list ID or a string of the form "username/listname" (replace spaces with dashes)')
...
class TwitterCommunityScraper(_TwitterAPIScraper):
    ...
    def _get_entity(self):
        ...
        obj = self._get_api_data('https://api.twitter.com/graphql/MO8cE7aTvaenXJX_teUGcA/CommunitiesFetchOneQuery',
                                 _TwitterAPIType.GRAPHQL, params=params)
        ...
        return Community(
            ...
            bannerUrl=community.get('custom_banner_media', community['default_banner_media'])['media_info'][
                'original_img_url'],
            **optKwargs,
        )
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data('https://api.twitter.com/graphql/Qvst9FkHq45wuqicCvMpVw/CommunityTweetsTimeline',
                                       _TwitterAPIType.GRAPHQL, params, paginationParams):
            if obj['data']['communityResults']['result']['__typename'] == 'CommunityUnavailable':
                _logger.warning('Community unavailable')
                break
            yield from self._graphql_timeline_instructions_to_tweets(
                obj['data']['communityResults']['result']['community_timeline']['timeline']['instructions'])
        ...
        for instruction in obj['timeline']['instructions']:
            ...
            for entry in instruction['addEntries']['entries']:
                ...
                for item in entry['content']['timelineModule']['items']:
                    ...
                    yield Trend(name=trend['name'], metaDescription=trend['trendMetadata'].get('metaDescription'),
